...
/**
 * Assert that an object is plain (no ObjectIds, Buffers, or Mongoose documents)
 * Use this BEFORE returning from Server Actions to catch serialization issues early
 * @param {any} obj - The object to check
 * @param {string} path - Current path in the object (for error messages)
 * @throws {Error} If non-plain object detected
 */
export function assertPlain(obj, path = 'root') {
    if (obj === null || obj === undefined) return;
    if (typeof obj !== 'object') return;
    
    // Check if it's a plain object/array
    const proto = Object.getPrototypeOf(obj);
    if (proto !== Object.prototype && proto !== Array.prototype && !Array.isArray(obj)) {
        console.error('❌ NON-PLAIN OBJECT AT:', path, obj);
        console.error('Object type:', obj.constructor?.name);
        console.error('Has toJSON:', typeof obj.toJSON === 'function');
        console.error('Has toObject:', typeof obj.toObject === 'function');
        throw new Error(`Non-plain object detected at ${path}. Object type: ${obj.constructor?.name}`);
    }
    
    // Recursively check nested objects
    for (const key in obj) {
        if (Object.prototype.hasOwnProperty.call(obj, key)) {
            assertPlain(obj[key], `${path}.${key}`);
        }
    }
}
 ...
 * Recursively serialize nested objects to handle ObjectIds in payload, etc.
 * @param {any} data
 * @returns {any}
 */
export function safeSerialize(data) {
    try {
        ...
        // Handle Buffer/ObjectId-like objects
        if (data && typeof data === 'object') {
            // Check if it's an ObjectId (has toString method and looks like ObjectId)
            ...
            // Handle Map (like activity payload)
