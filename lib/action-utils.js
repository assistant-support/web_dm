...
export async function runAction(
    ...
    try {
        ...
        // [FIX] Fetch latest role from DB to ensure admin privileges are up-to-date
        if (user?.externalUserId) {
            try {
                await connectDB();
                // Dynamic import to avoid circular dependencies if any, though model usually safe
                const AppUser = (await import('@/model/user.model.js')).default;
                const dbUser = await AppUser.findOne({ externalUserId: user.externalUserId }).select('role').lean();
                if (dbUser && dbUser.role) {
                    user.role = dbUser.role;
                }
            } catch (err) {
                console.warn('[runAction] Failed to fetch user role from DB:', err);
            }
        }
...
/**
 * Wrapper for MongoDB operations to catch and handle errors safely.
 * Returns null if the operation fails.
 * @template T
 * @param {Promise<T>} promise - The MongoDB promise to execute.
 * @returns {Promise<T|null>} The result or null if an error occurs.
 */
export async function safeMongo(promise) {
    try {
        return await promise;
    } catch (error) {
        console.error('[safeMongo] MongoDB operation failed:', error?.message || error);
        return null;
    }
}
