...
async function fetchUserInfoFromServer(token) {
    ...
    try {
        ...
        console.log('🌐 Fetched from server:', userInfo.email);
        return userInfo;
    } catch (error) {
...
export async function refreshTokenIfNeeded(session) {
    ...
    // Trong NextAuth OAuth flow, token refresh tự động được xử lý
    // Nếu cần custom refresh logic, implement ở đây
    return session;
}
...
export async function syncUserToLocalDB(userInfo) {
    ...
    if (!user) {
        ...
        if (needsUpdate) {
            console.log('🔄 Cập nhật user info:', userInfo.email);
            user.email = userInfo.email;
    ...
    try {
        ...
        if (session.accessToken) {
            try {
                ...
                console.log('✅ Đã lấy user info từ server (cached):', userInfo.email);
            } catch (error) {
                console.log('⚠️ Không thể fetch từ server, dùng session data:', error.message);
            }
        } else {
            console.log('⚠️ Không có accessToken, dùng session data');
        }
