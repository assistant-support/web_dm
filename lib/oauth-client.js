...
import { sanitizeTags } from '@/data/_shared/tags.js';
 ...
 * @returns {Promise<Object>} User info hoặc null nếu lỗi
 */
async function fetchUserInfoFromServer(token) {
    if (!token) {
        console.warn('⚠️  Token không được cung cấp');
        return null;
    }
    ...
    try {
        ...
        if (!response.ok) {
            console.warn(`⚠️  OAuth provider userinfo endpoint trả về HTTP ${response.status}`);
            return null;
        }
        ...
        console.warn('⚠️  Không thể lấy user info từ OAuth provider:', error.message);
        return null;
    }
}
 ...
 * @returns {Promise<Object|null>} User info (cached) hoặc null nếu lỗi
 */
export async function fetchUserInfo(token, userId) {
    if (!token || !userId) {
        console.warn('⚠️  Token hoặc userId không được cung cấp');
        return null;
    }
    const getCachedUserInfo = unstable_cache(
        ...
        [`user-info-${userId}`],
        (() => {
            const tagList = sanitizeTags([`user-${userId}`]);
            return tagList.length
                ? { revalidate: 60, tags: tagList }
                : { revalidate: 60 };
        })()
    );
 ...
 * @param {string} userInfo.sub - User ID từ OAuth (should be externalUserId)
...
export async function syncUserToLocalDB(userInfo) {
    ...
    // ⚠️ CRITICAL FIX: OAuth provider might return MongoDB _id in `sub`
    // We need to use externalUserId if available, otherwise use sub as fallback
    const userId = userInfo.externalUserId || userInfo.external_user_id || userInfo.sub;
    
    
    
    let user = await User.findOne({
        $or: [
            { oauthSub: userInfo.sub }, // Search by OAuth sub (might be _id)
            { externalUserId: userId }   // Search by externalUserId
        ]
    });
    ...
    if (!user) {
        ...
        user = await User.create({
            externalUserId: userId,     // Use resolved userId
            oauthSub: userInfo.sub,     // Keep original OAuth sub for reference
            email: userInfo.email,
            name: userInfo.name,
            avatar: userInfo.avt || userInfo.picture,
            role: 'member',
            isActive: true,
        });
        ...
        const needsUpdate =
            ...
            ((userInfo.picture || userInfo.avt) && user.avatar !== (userInfo.picture || userInfo.avt));
        ...
        if (needsUpdate) {
            ...
            if (userInfo.picture || userInfo.avt) {
                user.avatar = userInfo.picture || userInfo.avt;
            }
            await user.save();
        }

        // ⚠️ CRITICAL: Ensure externalUserId is set correctly
        if (!user.externalUserId || user.externalUserId === user._id.toString()) {
            // If externalUserId is missing or equals _id, update it
            user.externalUserId = userId;
            
            await user.save();
        }
        
        // Đảm bảo oauthSub được set
        if (!user.oauthSub) {
            user.oauthSub = userInfo.sub;
            await user.save();
        }
