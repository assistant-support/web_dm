 ...
 * @param {string} uid - User's external ID
...
export function canApproveSubtask(subtask, parentTask, uid) {
    ...
    if (parentTask.project && canManageProject(parentTask.project, uid)) {
        return true;
    }
 ...
 * @param {string} uid - The user's external ID.
 * @returns {boolean} True if user can create root task, otherwise false.
 * 
 * @example
 * const project = await Project.findById(projectId).lean();
 * if (canCreateTask(project, user.externalUserId)) {
 *   // Allow task creation
 * }
 */
export function canCreateTask(project, uid) {
    // Project Manager (owner or manager) có quyền tạo task gốc
    return canManageProject(project, uid);
}
 ...
 * if (canCreateSubtask(task, project, user.externalUserId)) {
 *   // Allow subtask creation
 * }
 */
export function canCreateSubtask(task, project, uid) {
    if (!task || !uid) return false;
