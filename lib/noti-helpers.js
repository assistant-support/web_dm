...
export async function notifyTeamMemberRemoved(teamName, removedUserId, managerUserIds, teamId) {
    try {
        ...
        await sendZalo(removedUserId, userMsg);
        await createInAppNotification({ 
        ...
        // 2. Notify managers
        const managerMsg = `Thành viên (ID: ${removedUserId}) vừa bị xóa khỏi nhóm: ${teamName}`;
        for (const managerId of managerUserIds) {
            if (String(managerId) === String(removedUserId)) continue;
            
            await sendZalo(managerId, managerMsg);
            await createInAppNotification({ 
                userId: managerId, 
                type: 'team.member.removed', 
                message: managerMsg, 
...
export async function notifyTeamMemberRoleUpdated(teamName, updatedUserId, newRole, managerUserIds, teamId) {
    try {
        ...
        await sendZalo(updatedUserId, userMsg);
        await createInAppNotification({ 
        ...
        // 2. Notify managers
        const managerMsg = `Thành viên (ID: ${updatedUserId}) trong nhóm ${teamName} vừa được cập nhật vai trò thành: ${newRole}`;
        for (const managerId of managerUserIds) {
...
export async function notifyProjectMemberRemoved(projectName, removedUserId, managerUserIds, projectId) {
    try {
        ...
        // 2. Notify managers
        const managerMsg = `Thành viên (ID: ${removedUserId}) vừa bị xóa khỏi dự án: ${projectName}`;
        for (const managerId of managerUserIds) {
            if (String(managerId) === String(removedUserId)) continue;
...
export async function notifyProjectMemberRoleUpdated(projectName, updatedUserId, newRole, managerUserIds, projectId) {
    try {
        ...
        // 2. Notify managers
        const managerMsg = `Thành viên (ID: ${updatedUserId}) trong dự án ${projectName} vừa được cập nhật vai trò thành: ${newRole}`;
        for (const managerId of managerUserIds) {
            if (String(managerId) === String(updatedUserId)) continue;
