...
// Minimal client component chỉ xử lý dropdown actions cho project
 ...
 * Chỉ xử lý dropdown menu và actions cho project
 * 
 * @param {Object} props
 * @param {Object} props.member - Member data
 * @param {string} props.projectId - Project ID
 */
export default function MemberRowActions({ member, projectId, isActive = true }) {
    ...
    const handleRemove = async () => {
        const result = await run(
            async () => await removeMemberAction(projectId, { userId: member.userId }),
            {
    ...
    const handleChangeRole = async (newRole) => {
        ...
        const result = await run(
            async () => await changeRole(projectId, { userId: member.userId, role: newRole }),
            {
    ...
    return (
        <>
            ...
            <DropdownMenu.Root>
                <DropdownMenu.Trigger asChild>
                    <button
                        ...
                        disabled={!isActive}
                        title={isActive ? 'Thao tác thành viên' : 'Project đã lưu trữ — thao tác bị vô hiệu'}
                    >
            ...
            <ConfirmDialog
                ...
                description="Bạn có chắc chắn muốn xóa thành viên này khỏi dự án? Họ sẽ mất quyền truy cập vào dự án này."
                confirmText="Xóa"
                cancelText="Hủy"
