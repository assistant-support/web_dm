// components/tasks/WorkflowEditor.client.js
// Simple workflow editor with drag & drop (no external graph library)
...
export default function WorkflowEditor({ task, subtasks = [], workflow, users }) {
    ...
    const handleNodeMouseDown = (e, nodeKey) => {
        ...
        setDraggingNode({
            ...
            startX: e.clientX,
            startY: e.clientY,
            nodeStartX: nodes.find(n => n.key === nodeKey).x,
            nodeStartY: nodes.find(n => n.key === nodeKey).y,
        });
    };
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            if (!result.ok) {
                setMessage({ type: 'error', text: result.message || 'Không thể lưu workflow' });
                return;
            }
            ...
            >
                ...
                >
                    ...
                    {edges.map((edge, idx) => {
                        ...
                        const x1 = fromNode.x + 120; // Center of node
                        const y1 = fromNode.y + 40;
                        const x2 = toNode.x + 120;
                        const y2 = toNode.y + 40;

                        // Calculate angle for arrow label
                        ...
                        return (
                            <g key={idx}>
                                ...
                                {/* Arrow direction indicator with background */}
                                ...
                                {/* Delete button - offset from center */}
                        ...
                        <div className={`bg-white border-2 rounded-lg shadow-lg overflow-hidden ${
                            ...
                            <div className={`px-3 py-2 ${
                                ...
                                <div className="flex items-center justify-between">
                                    ...
                                    <TaskStatusBadge status={node.status} size="xs" />
                                </div>
                            </div>
