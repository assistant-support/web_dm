...
// data/_shared/formatters.js
// Mục đích: Formatter dùng chung cho SSR/Client — định dạng ngày & nhãn task (status/priority).
...
const STATUS_LABELS = {
    ...
    waiting_assignee_confirm: 'Waiting confirm',
    in_progress: 'In progress',
    on_hold: 'On hold',
    completed_await_review: 'Await review',
    completed: 'Completed',
    cancelled: 'Cancelled',
    rejected: 'Rejected',
};
...
/**
 * Chuẩn hoá nhãn/ màu cho task — an toàn với enum UPPER_SNAKE_CASE (lower-case để map).
 * @param {Object} t
 * @returns {{ statusLabel:string, priorityLabel:string, priorityColor:string }}
 */
export function formatTask(t = {}) {
    const sKey = String(t.status || '').toLowerCase();
    const pKey = String(t.priority || '').toLowerCase();
    return {
        statusLabel: STATUS_LABELS[sKey] || (sKey ? sKey : '—'),
        priorityLabel: PRIORITY_LABELS[pKey] || (pKey ? pKey : '—'),
        priorityColor: PRIORITY_COLORS[pKey] || '#64748b',
    };
}
