// app/(auth)/(main)/search/teams/page.js
// Trang xem tất cả kết quả search teams

import { Suspense } from 'react';
import { getCurrentUser } from '@/lib/request-user';
import SearchTeamsClient from './SearchTeamsClient.client';
...
export async function generateMetadata({ searchParams }) {
    ...
    return {
        title: query ? `Tìm kiếm nhóm: "${query}" | Web DM` : 'Tìm kiếm nhóm | Web DM',
        description: `Kết quả tìm kiếm nhóm cho từ khóa "${query}"`
    };
}

export default async function SearchTeamsPage({ searchParams }) {
    const params = await searchParams;
    const query = params?.q || '';
