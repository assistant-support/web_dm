// app/(auth)/(main)/search/tasks/SearchTasksClient.client.js
'use client';
...
export default function SearchTasksClient({ initialQuery, userId, isAdmin }) {
    ...
    const fetchResults = useCallback(async (cursor = null, isLoadMore = false) => {
        ...
        try {
            const url = `/api/search/tasks?q=${encodeURIComponent(initialQuery)}${cursor ? `&cursor=${cursor}` : ''}`;
                        ...
                        {items.map((task) => (
                            ...
                            >
                                <div className="flex items-start gap-3">
                                    <div className="flex-shrink-0 w-10 h-10 rounded-lg bg-green-100 border border-green-200 flex items-center justify-center group-hover:bg-green-200 transition-colors">
